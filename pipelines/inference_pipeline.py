# pipelines/inference_pipeline.py
import os
import json
import joblib
import numpy as np
import pandas as pd
from datetime import datetime, timedelta
import requests
from dotenv import load_dotenv
import sys, os
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
from utils import pm25_to_aqi, calc_dew_point, categorize_aqi_label, fetch_recent_aqi_history
load_dotenv()

OPENWEATHER_KEY = os.getenv("OPENWEATHER_API_KEY")
LAT, LON        = 24.8607, 67.0011


# ─────────────────────────────────────────
# 1. FETCH CURRENT DATA
# ─────────────────────────────────────────
def fetch_current_data():
    """Fetch current AQI + weather from OpenWeather."""
    # Air pollution
    aqi_url = (
        f"http://api.openweathermap.org/data/2.5/air_pollution"
        f"?lat={LAT}&lon={LON}&appid={OPENWEATHER_KEY}"
    )
    aqi_r      = requests.get(aqi_url, timeout=10).json()
    components = aqi_r["list"][0]["components"]
    pm25       = components.get("pm2_5", 0)

    # ✅ FIX: EPA formula
    aqi = pm25_to_aqi(pm25)

    # Weather
    w_url  = (
        f"https://api.openweathermap.org/data/2.5/weather"
        f"?lat={LAT}&lon={LON}&appid={OPENWEATHER_KEY}&units=metric"
    )
    w_data = requests.get(w_url, timeout=10).json()
    temp     = w_data["main"]["temp"]
    humidity = w_data["main"]["humidity"]

    return {
        "aqi":         aqi,
        "pm25":        pm25,
        "pm10":        min(components.get("pm10", 0), 400),   # cap outlier
        "o3":          components.get("o3",  0),
        "no2":         components.get("no2", 0),
        "so2":         components.get("so2", 0),
        "co":          components.get("co",  0),
        "temp":        temp,
        "humidity":    humidity,
        "pressure":    w_data["main"]["pressure"],
        "wind":        w_data["wind"]["speed"],
        "dew":         calc_dew_point(temp, humidity),   # ✅ FIX
        "visibility":  float(w_data.get("visibility", 8000)),
        "cloud_cover": float(w_data["clouds"]["all"]),
    }


# ─────────────────────────────────────────
# 2. LOAD MODEL
# ─────────────────────────────────────────
def load_model():
    with open("models/best_model_name.json") as f:
        model_name = json.load(f)["name"]

    if model_name == "LSTM":
        import tensorflow as tf
        model = tf.keras.models.load_model("models/best_model.keras")
    else:
        model = joblib.load("models/best_model.pkl")

    scaler = joblib.load("models/scaler.pkl")
    with open("models/feature_names.json") as f:
        feature_names = json.load(f)

    print(f"   Loaded model: {model_name}")
    return model, scaler, feature_names, model_name


# ─────────────────────────────────────────
# 3. GENERATE FORECAST
# ─────────────────────────────────────────
def generate_forecast():
    print("🔮 Generating 3-day AQI forecast for Karachi...")

    current = fetch_current_data()
    model, scaler, feature_names, model_name = load_model()

    # ✅ FIX: Fetch real historical lag values
    print("   📡 Fetching AQI history for lag initialisation...")
    history = fetch_recent_aqi_history(n=7, api_key=OPENWEATHER_KEY)
    print(f"   History (last 7h): {history}")

    # prev_aqi keeps a rolling window used for lag/rolling features
    prev_aqi = history.copy()   # list of 7 values, oldest first

    print(f"   Current AQI: {current['aqi']}, PM2.5: {current['pm25']:.1f}, Temp: {current['temp']}°C")

    forecasts = []
    now       = datetime.now()

    for h in range(1, 73):
        future = now + timedelta(hours=h)

        row = {
            "pm25":            current["pm25"],
            "pm10":            current["pm10"],
            "o3":              current["o3"],
            "no2":             current["no2"],
            "so2":             current["so2"],
            "co":              current["co"],
            "humidity":        current["humidity"],
            "temp":            current["temp"],
            "pressure":        current["pressure"],
            "wind":            current["wind"],
            "dew":             current["dew"],
            "hour":            future.hour,
            "day":             future.day,
            "month":           future.month,
            "day_of_week":     future.weekday(),
            "is_weekend":      int(future.weekday() >= 5),
            "is_rush_hour":    int(future.hour in [7, 8, 9, 17, 18, 19]),
            "aqi_lag_1":       float(prev_aqi[-1]),
            "aqi_lag_2":       float(prev_aqi[-2]),
            "aqi_lag_3":       float(prev_aqi[-3]),
            "aqi_rolling_3":   float(np.mean(prev_aqi[-3:])),
            "aqi_rolling_7":   float(np.mean(prev_aqi[-7:])),
            "aqi_change_rate": float((prev_aqi[-1] - prev_aqi[-2]) / (prev_aqi[-2] + 1e-9)),
            "cloud_cover":     current["cloud_cover"],
        }

        X        = pd.DataFrame([[row[f] for f in feature_names]], columns=feature_names)
        X_scaled = scaler.transform(X)

        if model_name == "LSTM":
            X_input   = X_scaled.reshape((1, 1, X_scaled.shape[1]))
            predicted = float(model.predict(X_input, verbose=0)[0][0])
        else:
            predicted = float(model.predict(X_scaled)[0])

        # Prevent runaway drift — cap change at 10% per hour
        max_change = prev_aqi[-1] * 0.10
        predicted  = float(np.clip(predicted,
                                   prev_aqi[-1] - max_change,
                                   prev_aqi[-1] + max_change))
        predicted  = max(0, min(500, round(predicted, 1)))

        category, color = categorize_aqi_label(predicted)
        forecasts.append({
            "datetime": future.strftime("%Y-%m-%d %H:%M"),
            "date":     future.strftime("%b %d"),
            "hour":     future.hour,
            "aqi":      predicted,
            "category": category,
            "color":    color,
        })

        # Update rolling window
        prev_aqi.append(predicted)
        prev_aqi = prev_aqi[-7:]

    # ─── Daily summary ───
    df    = pd.DataFrame(forecasts)
    daily = df.groupby("date").agg(
        avg_aqi  =("aqi", "mean"),
        max_aqi  =("aqi", "max"),
        min_aqi  =("aqi", "min"),
        category =("category", lambda x: x.mode()[0]),
        color    =("color",    lambda x: x.mode()[0]),
    ).reset_index()

    current_cat, current_color = categorize_aqi_label(current["aqi"])
    result = {
        "current_aqi":      current["aqi"],
        "current_cat":      current_cat,
        "current_color":    current_color,
        "current_pm25":     current["pm25"],
        "current_pm10":     current["pm10"],
        "current_o3":       current["o3"],
        "current_no2":      current["no2"],
        "current_temp":     current["temp"],
        "current_humidity": current["humidity"],
        "current_wind":     current["wind"],
        "current_pressure": current["pressure"],
        "hourly":           forecasts,
        "daily":            daily.to_dict(orient="records"),
        "generated_at":     now.strftime("%Y-%m-%d %H:%M:%S"),
        "city":             "Karachi, Pakistan",
    }
    
    print(f"\n✅ Forecast generated!")
    for d in result["daily"]:
        print(f"   {d['date']:8s} → Avg: {d['avg_aqi']:.1f}, Max: {d['max_aqi']:.1f} ({d['category']})")

    return result


if __name__ == "__main__":
    generate_forecast()